// ==========================================================
// === 1. SYNCHRONOUS MOBILE DEVICE GATE (Runs First & Exits) ===
// ==========================================================
(function() {
    ...
    if (window.innerWidth <= MAX_UNSUPPORTED_WIDTH) {
        const unsupportedMessage = `
            <style>
                ...
                .mobile-gate-message { padding: 30px; max-width: 90%; border-radius: 12px; background: rgba(255, 255, 255, 0.05); border: 1px solid rgba(255, 255, 255, 0.1); box-shadow: 0 0 30px rgba(0, 0, 0, 0.5); }
                .mobile-gate-message h1 { font-size: 24px; margin-bottom: 10px; }
                .mobile-gate-message p { opacity: 0.8; }
            </style>
        ...
        // Overwrite the entire document content and prevent further JS execution
        document.documentElement.innerHTML = unsupportedMessage;
        
        // Use an aggressive exit to prevent the rest of the script from even parsing
        throw new Error("Mobile Gate triggered. Script execution stopped.");
    }
})();


// ==========================================================
// === 2. GLOBAL FUNCTION DEFINITIONS (Must be defined first) ===
// ==========================================================
...
/**
 * Updates the Profile Chip in the navbar with Supabase data.
 * @param {object | null} user - The Supabase user object.
 */
async function updateProfileAndUI(user) {
    const supabase = window.supabaseClient;
    console.log('[ProfileChip:DEBUG] updateProfileAndUI function started.', { signedIn: !!user });

    const wrapper = document.getElementById('chip-profile-wrapper');
    if (!wrapper) {
        console.error('[ProfileChip:ERROR] Element #chip-profile-wrapper not found.');
        return;
    }

    const img = wrapper.querySelector('.chip-img');
    const uname = wrapper.querySelector('.username');

    if (!user) {
        console.log('[ProfileChip:INFO] User is null. Setting defaults.');
        if (img) img.src = '/branding/default-pfp.png';
        if (uname) uname.textContent = 'Sign Up';
        return;
    }
    ...
    try {
        ...
        if (profile) {
            if (uname) uname.textContent = profile.username || 'User';
            if (img) img.src = profile.avatar_url || '/branding/default-pfp.png';
        } else {
            if (uname) uname.textContent = user.email.split('@')[0];
            if (img) img.src = '/branding/default-pfp.png';
        }
        ...
        console.error('[ProfileChip:ERROR] Failed to update navbar UI:', e);
    }
}
...
function applySettings() {
    ...
    if (settings.cloak_type === 'preset') {
        switch (settings.cloak_preset) {
            case 'gdoc': title = 'Untitled document - Google Docs'; favicon = '/uploads/cloaks/gdocs.png'; break;
            case 'gslides': title = 'Untitled presentation - Google Slides'; favicon = '/uploads/cloaks/gslides.png'; break;
            case 'gsheets': title = 'Untitled spreadsheet - Google Sheets'; favicon = '/uploads/cloaks/gsheets.png'; break;
            case 'desmoscalc': title = 'Desmos | Scientific Calculator'; favicon = '/uploads/cloaks/desmos.png'; break;
            case 'gdrive': title = 'Google Drive'; favicon = '/uploads/cloaks/gdrive.png'; break;
            case 'gassign': title = 'Google Assignments'; favicon = '/uploads/cloaks/gassign.png'; break;
            case 'blank': title = '\u200B'; favicon = '/uploads/cloaks/transparent.png'; break;
            case 'google': title = 'Google'; favicon = 'https://www.google.com/favicon.ico'; break;
            case 'calculator': title = 'Calculator - Google Search'; favicon = '/uploads/cloaks/google.png'; break;
        }
    ...
    window.onbeforeunload = settings.anti_tab_close 
        ? function() { return "Are you sure you want to leave?"; } 
        : null;
}
...
// ==========================================================
// === 3. SINGLE CONSOLIDATED DOMContentLoaded LISTENER ===
// ==========================================================

document.addEventListener('DOMContentLoaded', async () => {
    
    // --- A. Inject Dependencies (FA, Turnstile, Notification Container) ---

    // Notification Container MUST be created first before showing notifications
    let notifContainer = document.getElementById('moonNotifContainer');
    if(!notifContainer) {
      notifContainer = document.createElement('div');
      notifContainer.id = 'moonNotifContainer';
    ...
    window.getTurnstileToken = async () => {
        if (typeof window.turnstile === 'undefined') { console.warn('Turnstile not loaded yet'); return null; }
        try {
            const token = window.turnstile.getResponse();
            return token || null;
        } catch (e) { console.error('Error getting Turnstile token:', e); return null; }
    };
    ...
    Object.assign(navbarWrapper.style, {
      position: 'fixed', left: '20px', bottom: '20px', zIndex: 99999, pointerEvents: 'auto',
    ...
    Object.assign(profileWrapper.style, {
      position: 'fixed', right: '20px', bottom: '20px', zIndex: 99998, pointerEvents: 'auto',
      fontFamily: 'Inter, system-ui, -apple-system, "Segoe UI", Roboto, "Helvetica Neue", Arial'
    });
    ...
    style.textContent = `
      .chip-inner {
        z-index:999999; display:flex; align-items:center; gap:8px; width:50px; height:50px; padding:5px;
        border-radius:50%; background: rgba(255,255,255,0.06); backdrop-filter: blur(10px);
        -webkit-backdrop-filter: blur(10px); border:1px solid rgba(255,255,255,0.08);
        box-shadow:0 6px 20px rgba(0,0,0,0.45); transition: all 250ms cubic-bezier(.2,.9,.2,1);
      ...
      .chip-inner.expanded { width:auto; padding:10px 14px; min-width:120px; height:64px; border-radius:18px; background: rgba(255,255,255,0.04); border:1px solid rgba(255,255,255,0.08); }
      ...
      @media(max-width:420px) { .chip-inner.expanded { min-width:100px; } .chip-img { width:36px; height:36px; } .chip-link { width:38px; height:38px; font-size:16px; } }
    `;
    document.head.appendChild(style);
    ...
    // --- C. Supabase Client Setup ---
    
    // Inject UMD bundle if not present
    if (!window.supabase || typeof window.supabase.createClient !== 'function') {
      ...
      if (!existing) {
        ...
        s.onload = () => setTimeout(createAndDispatchClient, 20);
    ...
    // --- D. CORE INITIALIZATION AND LISTENER (Runs once Supabase is ready) ---
    
    window.addEventListener('supabase-ready', () => {
        const supabase = window.supabaseClient;
        if (!supabase) return;

        // 1. Initial Check: Get session, update UI, load settings
        supabase.auth.getSession()
            ...
            .catch(e => console.error('[ProfileChip:ERROR] Initial session check failed:', e));
            
        // 2. Real-time Listener: Listen for all future auth state changes
    ...
    if (localStorage.getItem(AGREED_KEY) !== "true") {
        ...
        legalStyle.textContent = `
            #legal-blur-overlay { position: fixed; inset: 0; backdrop-filter: blur(18px); -webkit-backdrop-filter: blur(18px); background: rgba(0,0,0,0.55); z-index: 999999999; }
            #legal-popup { 
                position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); width: 92%; max-width: 460px; 
                background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.12); border-radius: 18px; 
                padding: 26px; color: #fff; font-family: "Inter Tight", system-ui, sans-serif; text-align: center; 
                backdrop-filter: blur(24px); -webkit-backdrop-filter: blur(24px); box-shadow: 0 18px 60px rgba(0,0,0,0.6); 
                animation: fadeInLegal .35s cubic-bezier(.2,.9,.2,1) both; z-index: 9999999999; 
            }
            @keyframes fadeInLegal { from { opacity:0; transform:translate(-50%, -46%) scale(.96); } to { opacity:1; transform:translate(-50%, -50%) scale(1); } }
            #legal-popup h2 { font-size: 22px; margin-bottom: 10px; }
            #legal-popup p { font-size: 14px; opacity: 0.85; margin-bottom: 18px; line-height: 1.5; }
            #legal-agree-row { display: flex; align-items: center; gap: 10px; font-size: 14px; opacity: 0.9; margin-bottom: 18px; justify-content: center; }
            #legal-continue-btn { 
                background: rgba(255,255,255,0.08); border: 1px solid rgba(255,255,255,0.14); border-radius: 12px; 
                padding: 12px 18px; font-size: 15px; color: #fff; cursor: pointer; transition: 0.2s; width: 100%; 
            }
            #legal-continue-btn:disabled { opacity: 0.45; cursor: not-allowed; }
            #legal-continue-btn:not(:disabled):hover { transform: translateY(-3px); background: rgba(255,255,255,0.12); }
        `;
        document.head.appendChild(legalStyle);

        // ===== Inject Blur Overlay & Popup =====
        ...
        btn.addEventListener("click", () => {
            ...
            setTimeout(() => { popup.remove(); blurOverlay.remove(); }, 300);
            window.showMoonNotification({ title: "Thank you!", body: "Your agreement has been saved.", icon: "fa-solid fa-circle-check", duration: 3000 });
        });
    }

    // --- F. Initial Notification Display ---
